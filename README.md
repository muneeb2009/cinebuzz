...
## Files
- app/page.tsx: reusable picker and recommendation components, marketing sections, client-side booking and demo analytics.
...
No backend booking, payment gateway, authentication, inventory API, ticket validation or outbound form submission. Details fields are discarded on leaving the details screen. Analytics contain only event names and timestamps in sessionStorage. Closing the session or clearing browser data clears counters. Reload resets in-progress booking. No secrets are required. A generated DEMO reference is presentation-only.
